...
class ArmorIQClient:
    ...
    DEFAULT_BACKEND_ENDPOINT = "https://staging-api.armoriq.io"
    ...
    def __init__(
        ...
        # Endpoint resolution: param > env > default (routed by API-key prefix)
        self.iap_endpoint = (
            iap_endpoint
            or os.getenv("IAP_ENDPOINT")
            or (
                (self.ARMORCLAW_IAP_ENDPOINT if use_prod else self.LOCAL_ARMORCLAW_IAP_ENDPOINT)
                if is_armorclaw
                else (self.DEFAULT_IAP_ENDPOINT if use_prod else self.LOCAL_IAP_ENDPOINT)
            )
        )
        self.default_proxy_endpoint = (
            ...
            or (
                (self.ARMORCLAW_PROXY_ENDPOINT if use_prod else self.LOCAL_ARMORCLAW_PROXY_ENDPOINT)
                if is_armorclaw
                else (self.DEFAULT_PROXY_ENDPOINT if use_prod else self.LOCAL_PROXY_ENDPOINT)
            )
        )
        self.backend_endpoint = (
            ...
            or (
                (self.ARMORCLAW_BACKEND_ENDPOINT if use_prod else self.LOCAL_ARMORCLAW_BACKEND_ENDPOINT)
                if is_armorclaw
                else (self.DEFAULT_BACKEND_ENDPOINT if use_prod else self.LOCAL_BACKEND_ENDPOINT)
            )
        )
